                ...
                chunk.indexWhere(_  ===  SPACE) match {
                  case Some(idx) =>
                    ...
                    if (validMethods contains method)
                      go(restOfStream.cons(chunk.drop(idx + 1)), step + 1, request.copy(method = method), Array.empty)
                    else Pull.raiseError(new IllegalArgumentException(s"Invalid method $method"))
                  case None =>
