...
trait Generators {
  ...
  implicit def expenseArb(implicit
    ...
    Arbitrary(for {
      ...
      participants <-
        Gen.nonEmptyListOf(arbPerson.arbitrary filter (_ != payer))
    } yield Expense.unsafeCreate(payer, money, participants))
  ...
  implicit def personOpArb[A](implicit
    ...
    Arbitrary {
      for {
        a <- arbA.arbitrary
        state <- arbPersonState.arbitrary
      } yield State[PersonState, A](_ => (state, a))
    }
