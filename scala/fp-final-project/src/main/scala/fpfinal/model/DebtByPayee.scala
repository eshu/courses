...
object DebtByPayee {
  ...
  def fromExpense(expense: Expense): DebtByPayee = {
    val amount = expense.amountByParticipant
    new DebtByPayee(expense.participants.toIterable.map(_ -> amount).toMap)
  }
  ...
  ): Eq[DebtByPayee] = eqMap.contramap(_.debtByPayee)
  ...
  ): Monoid[DebtByPayee] = monoidMap.imap(new DebtByPayee(_))(_.debtByPayee)
