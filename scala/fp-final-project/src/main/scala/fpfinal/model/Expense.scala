...
object Expense {
  ...
  def create(
    ...
    (
      Validated.condNec(
        participants.forall((_: Person) =!= payer),
        payer,
        "participants should not contain payer"
      ),
      Validations.nonEmptySet(participants)
    ).mapN(new Expense(_, amount, _))
  ...
  implicit def eqExpense(implicit
    ...
    Eq.instance((a, b) =>
      a.payer === b.payer && a.amount === b.amount && a.participants === b.participants
    )
  ...
  /**
    ...
    */
  implicit val showExpense: Show[Expense] = (expense: Expense) =>
    f"Expense[Payer=${expense.payer.show},Amount=${expense.amount.show},Participants=${expense.participants mkString_ ","}]"
}
