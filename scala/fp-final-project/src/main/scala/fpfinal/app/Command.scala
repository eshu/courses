...
object AddExpenseCommand extends Command {
  ...
  def execute(): AppOp[SuccessMsg] = {
    ...
    def readParticipants(): AppOp[List[String]] =
      ME.tailRecM(Nil: List[String])(participants => {
        for {
          env <- readEnv
          line <- env.console.readLine("Enter participant's name: ").toAppOp
        } yield line match {
          case "END" => Right(participants)
          case name  => Left(name :: participants)
        }
      })
    ...
    def validateData(
      ...
      (
        Validations.nonEmptyString(payer),
        Validations.double(amount),
        participants.map(Validations.nonEmptyString).sequence
      ) mapN { AddExpenseData }
    ...
    def findPerson(name: String): AppOp[Person] =
      for {
        env <- readEnv
        optPerson <- env.personService.findByName(name).toAppOp
        person <- ME.fromOption(optPerson, s"Person not found: $name")
      } yield person
...
case object AddPersonCommand extends Command {
  ...
  override def execute(): AppOp[SuccessMsg] = {
    ...
    for {
      env <- readEnv
      personData <- readData()
      person <- Person.create(personData.name).toAppOp
